...
namespace CpuScheduler
{
    ...
    public partial class CpuSchedulerForm : Form
    {
        ...
        /// <summary>
        ///  Handles comparison panel navigation
        /// </summary>
        private void btnComparison_Click(object sender, EventArgs e)
        {
            ShowPanel(comparisonPanel);
            sidePanel.Height = btnComparison.Height;
            sidePanel.Top = btnComparison.Top;
        }
        ...
        {
            comparisonPanel.Text = @"Welcome to CPU Scheduler Simulator
...
4. Compare algorithm behaviors and outcomes";
        ...
        private List<SchedulingResult> RunSJTRFAlgorithm(List<ProcessData> processes)
        {
            ...
            var currentTime = 0;
            int remainingProcesses = processes.Count;

            var availableProcesses = new List<ProcessData>();
            Dictionary<String, int> startTimes = new Dictionary<String, int>();
            Dictionary<String, int> finishTimes = new Dictionary<String, int>();
            Dictionary<String, int> remainingBurstTimes = new Dictionary<String, int>();
            foreach (var p in processes)
            {
                startTimes.Add(p.ProcessID, -1);
                finishTimes.Add(p.ProcessID, -1);
            }
        ...
        private List<SchedulingResult> RunHRRNAlgorithm(List<ProcessData> processes)
        {
            ...
            Dictionary<String, int> startTimes = new Dictionary<String, int>();
            Dictionary<String, int> finishTimes = new Dictionary<String, int>();
            HashSet<String> addedPIDS = new HashSet<String>();
            foreach (var p in processes)
            {
                startTimes.Add(p.ProcessID, -1);
                finishTimes.Add(p.ProcessID, -1);
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            // 2. Average Turnaround Time (ATT) - sum of al
            // l turnaround times / number of processes  
        ...
        private void ApplyDarkTheme()
        {
            ...
            // Text boxes
            comparisonPanel.BackColor = Color.FromArgb(37, 37, 38);
            comparisonPanel.ForeColor = Color.FromArgb(241, 241, 241);
        ...
        private void ApplyLightTheme()
        {
            ...
            // Text boxes
            comparisonPanel.BackColor = SystemColors.Window;
            comparisonPanel.ForeColor = SystemColors.WindowText;
        ...
        /// <summary>
        /// Executes the Shortest Job Time Remaining First algorithm using DataGrid data.
        /// STUDENTS: Updated to use GetProcessDataFromGrid() instead of prompts
        /// Each process gets a time quantum (default 4) before switching to next process
        /// </summary>
        ...
        private void btnHRRN_Click(object sender, EventArgs e)
        {
            ...
            if (processData.Count > 0)
            {
                ...
                // Update Results tab with detailed scheduling results
                DisplaySchedulingResults(results, "HRRN - Highest Response Ratio First");
        ...
        private void comparisonPanel_Paint(object sender, PaintEventArgs e)
        {
